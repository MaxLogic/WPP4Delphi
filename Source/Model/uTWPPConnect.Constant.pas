...
Const
  ...
  FrmConsole_JS_GetBatteryLevel         = ''; //'window.WAPI.getBatteryLevel();'; //Nao Habilitar Funçao deprecated GetBatteryLevel
...
//FrmConsole_JS_refreshOnlyQRCode       = ' interval = window.setInterval(async function() {new Promise((resolve, reject) =>{let all = []; all = document.querySelector("button"); if (all) { if (all.text.includes("recarregar")) { all.click() } } })}, 60000);';
  ...
  FrmConsole_JS_MonitorChatLoadComplete =
    ...
    '  var targetElement = document.querySelector(''#pane-side''); // Altere o seletor conforme necessÃ¡rio' + sLineBreak +
    '' + sLineBreak +
    ...
    '          obs.unobserve(entry.target); // Para observar sÃ³ uma vez' + sLineBreak +
    '        }' + sLineBreak +
    '      });' + sLineBreak +
    '    }, {' + sLineBreak +
  ...
  FrmConsole_JS_Monitor_Received_Message_Socket2 =
    ...
    '' + sLineBreak +
    '    try {' + sLineBreak +
    '        // Verifica se o campo ''from'' ou ''id.remote'' contÃ©m ''@newsletter''' + sLineBreak +
...
resourcestring
  ...
  Text_FrmClose_Label                  = '';
  Text_FrmClose_WarningClose           = 'Fechar a aplicaçao?';
